...
function isApiCredentials(credentials: ActionRelayerParams | ApiRelayerParams): credentials is ApiRelayerParams {
...
function validatePayload(payload: RelayerTransactionPayload) {
  if (isEIP1559Tx(payload) && BigInt(payload.maxFeePerGas as BigintLike) < BigInt(payload.maxPriorityFeePerGas as BigintLike)) {
